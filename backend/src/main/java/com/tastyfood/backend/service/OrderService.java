...
public class OrderService {
    ...
    public Order createOrder(Order order, List<OrderItem> orderItems, DeliveryAddress deliveryAddress) {
        ...
        // Ensure delivered_at is null for new orders (will be set later when order is
        // delivered)
    ...
    /**
     * Save order to queue: update status to OUT_FOR_DELIVERY, assign driver, and
     * set estimated delivery time if needed
     * 
    ...
    public Order saveOrderToQueue(String orderId, String driverFullName) {
        ...
        // Convert Instant to milliseconds (SQLite stores timestamps as milliseconds
        // since epoch)
        if (estimatedDeliveryTime != null) {
            long estimatedDeliveryTimeMs = estimatedDeliveryTime.toEpochMilli();
            entityManager
                    .createNativeQuery(
                            "UPDATE orders SET driver_id = ?, estimated_delivery_time = ? WHERE order_id = ?")
